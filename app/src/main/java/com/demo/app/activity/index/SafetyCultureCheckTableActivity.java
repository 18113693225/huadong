...
public class SafetyCultureCheckTableActivity extends BaseActivity implements OnClickListener{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.index_safety_culture_check_xsktable_layout);
		TitleCommon.setTitle(this, null, "安全检查巡视卡",SafetyCultureActivity.class, true);
		sp = getSharedPreferences(Constents.SHARE_CONFIG, Context.MODE_PRIVATE);
		exist_dm_dj_tables = (Button) this.findViewById(R.id.exist_sc_check_tables);
		new_dm_dj_tables = (Button) this.findViewById(R.id.new_sc_check_tables);
		exist_dm_dj_tables.setOnClickListener(this);
		new_dm_dj_tables.setOnClickListener(this);
		dmDjTablesListView = (ListView)this.findViewById(R.id.sccheckTablesListView);
		...
		dmDjTablesListView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
				...
				if(Integer.parseInt(data.get(arg2).get("seeItemAuth").toString())==0){
					Toast.makeText(SafetyCultureCheckTableActivity.this, "对不起，权限不足，请联系管理员!", Toast.LENGTH_SHORT).show();
					return;
				}
				Intent i = new Intent();
				...
				i.setClass(SafetyCultureCheckTableActivity.this, SafetyCultureCheckListShowActivity.class);
				startActivity(i);
			}
		});
	}
	...
	public List<Map<String, Object>> getData() {
		...
		data.clear();
		NetworkData.getInstance().safetyTrainingList(new NetworkResponceFace() {
...
//					result = URLDecoder.decode(result,"utf-8");
					...
					for(int i=0;i<rel.length();i++){
						...
						map.put("uid", obj.get("fill_in_user_id").toString());
	...
	public void onClick(View v) {
		// TODO Auto-generated method stub
		switch (v.getId()) {
		case R.id.exist_sc_check_tables:
			new_dm_dj_tables.setBackgroundResource(R.drawable.button_franchise_shape1);
			exist_dm_dj_tables.setBackgroundResource(R.drawable.button_franchise_shape);
			break;
		case R.id.new_sc_check_tables:
			...
			Intent i = new Intent();
			i.setClass(SafetyCultureCheckTableActivity.this, SafetyCultureCheckActivity.class);
			startActivity(i);
			break;
		default:
			break;
