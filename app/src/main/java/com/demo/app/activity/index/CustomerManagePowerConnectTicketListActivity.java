...
//客户经理权力列表
public class CustomerManagePowerConnectTicketListActivity extends BaseActivity implements OnClickListener{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentViewWithoutTitle(R.layout.index_customer_manager_powerconnectticket_list_layout);
		TitleCommon.setTitle(this,null, "停（送）电联系单", CustomerManagerActivity.class,true);
		sp = getSharedPreferences(Constents.SHARE_CONFIG, Context.MODE_PRIVATE);
		exist_workconnect_tables = (Button) this.findViewById(R.id.exist_cm_powerconnect_tables);
		new_workconnect_tables = (Button) this.findViewById(R.id.new_cm_powerconnect_tables);
		exist_workconnect_tables.setOnClickListener(this);
		new_workconnect_tables.setOnClickListener(this);
		pmWorkconnectTablesListView = (ListView)this.findViewById(R.id.cmPowerconnectTablesListView);
...
//				Intent i = new Intent();
//				i.putExtra("id", id);
//				i.setClass(CustomerManagePowerConnectTicketListActivity.this, CustomerManagerPowerConnectTicketShowActivity.class);
//				startActivity(i);
				if(Integer.parseInt(data.get(arg2).get("seeItemAuth").toString())==0){
					Toast.makeText(CustomerManagePowerConnectTicketListActivity.this, "对不起，权限不足，请联系管理员!", Toast.LENGTH_SHORT).show();
					return;
				}
				String id = data.get(arg2).get("id").toString();
				Intent i = new Intent();
	...
	public List<Map<String, Object>> getData() {
		...
		data.clear();
		NetworkData.getInstance().stopOrSendContactList(new NetworkResponceFace() {
...
//					result = URLDecoder.decode(result,"utf-8");
	...
	public void onClick(View v) {
		// TODO Auto-generated method stub
		switch (v.getId()) {
		case R.id.exist_cm_powerconnect_tables:
			new_workconnect_tables.setBackgroundResource(R.drawable.button_franchise_shape1);
			exist_workconnect_tables.setBackgroundResource(R.drawable.button_franchise_shape);
			break;
		case R.id.new_cm_powerconnect_tables:
			...
			Intent i = new Intent();
			i.setClass(CustomerManagePowerConnectTicketListActivity.this, CustomerManagerPowerConnectTicketActivity.class);
			startActivity(i);
			break;
		default:
			break;
